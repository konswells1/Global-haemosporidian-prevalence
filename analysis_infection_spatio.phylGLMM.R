...
####################
# Analysis of bird infection with haemosporidian parasites at multiple sites, using spatio-phylogenetic GLMM with an R-INLA approach
...
setwd(paste0(dir.data, "/data_update_2011/"))
# Avian Malaria records
DatAVM <- read.csv("Data.AVM.Global_Host.Infection_201110.csv", header=T)

# Location-specific covariates
DatEnv <- read.csv("Data.AVM.Global_Location.Covariates_201110.csv", header=T)

# Host trait data
DatHost <- read.csv("Data.AVM.Global_Host.Traits_201110.csv", header=T)

match_host2avm <- match(DatAVM$Host, DatHost$Species)
match_env2avm <- match(paste(DatAVM$Latitude, DatAVM$Longitude), paste(DatEnv$lat, DatEnv$long))

# Vectors of point coordinates
pts_id <- DatEnv$ID.pts
pts_lat <- DatEnv$lat
pts_long <- DatEnv$long
NPoints <- dim(DatEnv)[1]
pts_realm <- DatEnv$Realm
pts_realm[which(pts_realm=="Oceanina")] <- "Oceania"


Dist.pts <- as.matrix(dist(data.frame(pts_long, pts_lat), method="euclidean", diag=TRUE, upper=TRUE))
plot(sort(Dist.pts[upper.tri(Dist.pts)]))
...
#Scaled covariates within dataframe
...
DatMerge$prop_snow_ice_sc <-  as.numeric(scale(DatMerge$prop_snow_ic))
...
DatMerge$Bodymass_log10.sc <-  as.numeric(scale(log10(DatMerge$Bodymass)))
...
host_spname <- sort(unique(DatMerge$Host))
host_family <- DatMerge$BLFamilyLatin[match(host_spname, DatMerge$Host)]
nhost <- length(host_spname)
host_id <- 1:nhost

family_name <- sort(unique(DatMerge$BLFamilyLatin))
nfamily <- length(family_name)
family_id <- 1:nfamily

DatMerge$Host_id <- host_id[match(DatMerge$Host, host_spname)]
DatMerge$Family_id <- family_id[match(DatMerge$BLFamilyLatin, family_name)]

DatMerge$Realm <- ifelse(!is.na(DatMerge$Realm), DatMerge$Realm, 'others')
DatMerge$Realm[which(DatMerge$Realm=="Oceanina")] <- "Oceania"
DatMerge$RealmID <- as.numeric(as.factor(DatMerge$Realm))

realm_name <- sort(unique((DatMerge$Realm)))
nrealm <- length(realm_name)

sel.DatMerge_realm.Leuc <- which(is.na(match(DatMerge$Realm, c("Australian", "Oriental", "Oceania"))))
DatMergeLeuc <- DatMerge[sel.DatMerge_realm.Leuc,]

########
# Compute species-level and family-level phylogenetic distance matrices

# Array to store 100 species-level phylogenetic precion matrices
ntree <- 10  ##ntree <- 100 Many replicates do not change familylevel phylognetic tree!
...
fu_extract.randeff.realm <- function(mod){
	mod <- mod
	reffno <- which(names(mod$summary.random)=="RealmID" | names(mod$summary.random)=="realm")
	...
	if(nrow(randeff)==nrealm){
		randeff.realm[,2:4] <- randeff
	}
	if(nrow(randeff)==6){
		randeff.realm[is.na(match(realm_name, c("Australian", "Oceania", "Oriental"))),2:4] <- randeff
	}
	rm(mod)
	randeff.realm
}
...
fu_extract.randeff.pts <- function(mod){
	...
	if(nrow(randeff)==npts){
		randeff.pts[,2:4] <- randeff
	}
	if(nrow(randeff)<npts){
		randeff.pts[is.na(match(pts_realm, c("Australian", "Oceania", "Oriental"))),2:4] <- randeff
	}
	rm(mod)
	randeff.pts
}
...
f_realm_Plas <- as.formula("Plas_pa ~ -1 + f(RealmID, model='iid')")
f_realm_ParaHaem <- as.formula("ParaHaem_pa ~ -1 + f(RealmID, model='iid')")
f_realm_Haem <- as.formula("Haem_pa ~ -1 + f(RealmID, model='iid')")
f_realm_Leuc <- as.formula("Leuc_pa ~ -1 + f(RealmID, model='iid')")
...
mod_realm_Haem <- inla(f_realm_Haem,family="binomial",data=DatMerge, control.predictor=list(compute=TRUE), control.compute=list(dic=TRUE,cpo=TRUE))
...
f_pts_Plas <- as.formula("Plas_pa ~ -1 + f(ID.pts, model='iid')")
f_pts_ParaHaem <- as.formula("ParaHaem_pa ~ -1 + f(ID.pts, model='iid')")
f_pts_Haem <- as.formula("Haem_pa ~ -1 + f(ID.pts, model='iid')")
f_pts_Leuc <- as.formula("Leuc_pa ~ -1 + f(ID.pts, model='iid')")
...
pred_fixeff <- paste("1 + abslat_sc + elevation_sc + ",
			...
			"ForStrat.canopy_sc + Bodymass_log10.sc + bird.richness_sc")
...
mod_fixeff_Haem <- inla(f_fixeff_Haem,family="binomial",data=DatMerge, control.predictor=list(compute=TRUE, hyper=pc.prec), control.compute=list(dic=TRUE,cpo=TRUE))
...
mod_phyl.0_Haem <- inla(f_phyl.0_Haem,family="binomial",data=DatMerge, control.predictor=list(compute=TRUE), control.compute=list(dic=TRUE,cpo=TRUE), control.inla=list(cmin=0, int.strategy="eb", strategy="adaptive"))
mod_phyl.0_Leuc <- inla(f_phyl.0_Leuc,family="binomial",data=DatMerge[sel.DatMerge_realm.Leuc,], control.predictor=list(compute=TRUE), control.compute=list(dic=TRUE,cpo=TRUE), control.inla=list(cmin=0, int.strategy="eb", strategy="adaptive"))


#######
# phylo-cov-GLMM: fixed effects, with phylogeny and year as random effects but NOT zooregion

pred_phyl.Cov <- paste("-1 + abslat_sc +  elevation_sc + ",
			"bio1_sc + bio12_sc + bio14_sc + bio15_sc + ",
			"NDVI.mean_sc + NDVI.SD_sc + prop_treecover_sc + prop_water_sc +",
			"ForStrat.canopy_sc + Bodymass_log10.sc + bird.richness_sc + f(Family_id, model='generic0', Cmatrix=phyl.prec_family) +",
			"f(Year, model='iid')")
...
mod_phyl.Cov_Haem <- inla(f_phyl.Cov_Haem,family="binomial",data=DatMerge, control.predictor=list(compute=TRUE, hyper=pc.prec), control.compute=list(dic=TRUE,cpo=TRUE), control.inla=list(cmin=0, int.strategy="eb", strategy="adaptive"))
...
plot_mesh <- function(){
	...
	points(pts_long, pts_lat, pch=19, col = brewer.pal(n = nrealm, name = "Set1")[as.numeric(as.factor(pts_realm))])
	legend("bottom", legend=realm_name, pch = 19, col = brewer.pal(n = nrealm, name = "Set1"), xpd = TRUE, horiz = TRUE, inset = c(0, -0.05), cex= 0.80 )
	}
plot_mesh ()
...
# Mesh for Leucocytozoon without AustralAsian records
...
stack_Plas <- inla.stack(
  ...
  A = list(A,1,1,1,1,1),   # One A for each effect
  effects = list(
	sp_id = 1:spde$n.spde,
	family_id = DatMerge$Family_id,
	year = DatMerge$Year, 
	zooreg = DatMerge$RealmID,
...
f_spde <- y ~ -1 + abslat + elevation + bio1 + bio12 + bio14 + bio15 + 
			ndvi.mean + ndvi.sd + LC.forest + LC.wetland + forstrat.canopy + log.bm + bird.richness_sc +  
...
covar_str <- c("abslat", "elevation", "bio1", "bio12", "bio14", "bio15", "ndvi.mean", "ndvi.sd", "LC.forest", "LC.wetland", "forstrat.canopy", "log.bm", "bird.richness_sc")
covar_name <- c("Equator dist", "Elevation", "Temp mean", "Rain total", "Rain min", "Rain var", "NDVI mean", "NDVI sd", "Forest cover", "Wetland cover", "Canopy foraging", "Body mass", "Bird species richness")
...
nhost <- length(par_str )

# Tables of model DICs and cpo 
tble_DIC <- tble_cpo.mean <- tble_cpo.sd <-matrix(NA, nrow=nmodels, ncol=nhost)
...
#### 
#  Overall average infection probabilities for different parasite species
tble_inf0 <- rbind(
...
post_infprop.realm <- post_infprop.realm %>%
  mutate(mean = replace(mean, genus=="Leucocytozoon" & (realm=="Australian" | realm=="Oriental" | realm=="Oceanian"), NA)) %>%
  mutate(CI.1 = replace(CI.1, genus=="Leucocytozoon" & (realm=="Australian" | realm=="Oriental" | realm=="Oceanian"), NA)) %>%
  mutate(CI.2 = replace(CI.2, genus=="Leucocytozoon" & (realm=="Australian" | realm=="Oriental" | realm=="Oceanian"), NA))

# Highest regional infection probabilties in Saharo-Arabian region for common parasites
post_infprop.realm %>% filter(realm=="Saharo-Arabian"  & species!="Haemoproteus")
# Infection < 10% for common genera
post_infprop.realm %>% filter(species!="Haemoproteus" & CI.2<0.1)
# Highest/lowest regional infection with Haemoproteus
arrange(post_infprop.realm %>% filter(species=="Haemoproteus"), CI.1)
arrange(post_infprop.realm %>% filter(species=="Haemoproteus"), CI.2)
...
tiff("plot_inf.realm.tiff", width = 3.307*3, height = 3.307*2, units = 'in', res = 600); plot_inf.realm
dev.off()
...
post_infprop.pts <- rbind(
	cbind(fu_extract.randeff.pts(get(paste0("mod_pts_", par_str[1])))[,2:4], genus=par_name[1], pointID=pts_id, long = pts_long, lat = pts_lat, realm=pts_realm),
	cbind(fu_extract.randeff.pts(get(paste0("mod_pts_", par_str[2])))[,2:4], genus=par_name[2], pointID=pts_id, long = pts_long, lat = pts_lat, realm=pts_realm),
	cbind(fu_extract.randeff.pts(get(paste0("mod_pts_", par_str[3])))[,2:4], genus=par_name[3], pointID=pts_id, long = pts_long, lat = pts_lat, realm=pts_realm),
	cbind(fu_extract.randeff.pts(get(paste0("mod_pts_", par_str[4])))[,2:4], genus=par_name[4], pointID=pts_id, long = pts_long, lat = pts_lat, realm=pts_realm))
...
post_infprop.pts %>% group_by(species) %>% summarise(n = length(which(abs(CIsize)<0.10)))

# Locations with highest infection probabilities
hotspots_local <- rbind(
top_n(post_infprop.pts %>% filter(genus==par_name[1]), 3, CI.1),
top_n(post_infprop.pts %>% filter(genus==par_name[2]), 3, CI.1),
top_n(post_infprop.pts %>% filter(genus==par_name[3]), 3, CI.1),
top_n(post_infprop.pts %>% filter(genus==par_name[4]), 3, CI.1))
# infection rates at hotspots
range(c(hotspots_local$CI.1, hotspots_local$CI.2)); range(hotspots_local$CI.2)

plotfu_infprop.pts <- function(species, threshold) {
	sel_infprop <- which(post_infprop.pts$species==species & post_infprop.pts$CIsize<=threshold)
	sel_point <- post_infprop.pts$pointID[sel_infprop] 
	ggplot(data = worldmap) + geom_sf() + theme_classic() + ylim(-55, 80) +
	geom_point(data=data.frame(x=pts_long[sel_point], y=pts_lat[sel_point], cols=post_infprop.pts[sel_infprop,"mean"]), aes(x=x, y=y, color =cols)) + scale_color_gradient(low = "blue", high = "red", name="Posterior mean") +
	ggtitle(paste(species)) + ylab("") + xlab("")
} 

plot_infprop.pts <- ggarrange(plotfu_infprop.pts(par_name[1], 0.2), plotfu_infprop.pts(par_name[2], 0.2), plotfu_infprop.pts(par_name[3], 0.2), plotfu_infprop.pts(par_name[4], 0.2), ncol=2, nrow=2, common.legend = F)
plot_infprop.pts


tiff("plot_infprop.pts.tiff", width = 3.307*3, height = 3.307*2, units = 'in', res = 600); plot_infprop.pts
dev.off()
...
plot_phyleffect <- 
ggarrange(
fu_plot_randeff.family.2(get(paste0("mod_spde.varcoeff_", par_str [1])), paste(par_name[1])),
fu_plot_randeff.family.2(get(paste0("mod_spde.varcoeff_", par_str [2])), paste(par_name[2])),
fu_plot_randeff.family.2(get(paste0("mod_spde.varcoeff_", par_str [3])), paste(par_name[3])),
fu_plot_randeff.family.2(get(paste0("mod_spde.varcoeff_", par_str [4])), paste(par_name[4])))


tiff("plot_phyleffect.tiff", width = 3.307*3, height = 3.307*2, units = 'in', res = 600); plot_phyleffect
dev.off()
...
# post_fixeff = post_fixeff %>% filter(significant==1)


plot_fixeff <- ggplot(post_fixeff, aes(x=covar, y=mean, group=genus, color=genus)) + 
  geom_hline(yintercept=0, linetype="dashed", color = "black") +
  geom_point(position=position_dodge(0.4), size=2, alpha=(0.2+post_fixeff$significant*3))+ xlab("Covariate") + ylab("Posterior estimate") +
  geom_errorbar(aes(ymin = CI.1, ymax = CI.2), size=0.8, width=0.3, position=position_dodge(0.4), alpha=(0.2+post_fixeff$significant*3)) + scale_color_brewer(palette="Dark2") +
  theme(axis.text.x = element_text(size = 12, angle = 90, hjust = 1)) + ylim(-2,2)
plot_fixeff

# Odds ratios for 'significnat effects
post_fixeff_sign <- post_fixeff %>% filter(significant==1)
post_fixeff_sign.odds <- post_fixeff_sign[, c("covar", "species", "mean", "CI.1", "CI.2")] 
post_fixeff_sign.odds[, c("mean", "CI.1", "CI.2")] <- round((exp(post_fixeff_sign.odds[, c("mean", "CI.1", "CI.2")])),2)

pf=post_fixeff_sign.odds
post_fixeff_sign.odds_print <- data.frame(Variable=pf$covar, Species=pf$species, Odds_ratio=paste0(pf$mean, " (", pf$CI.1, " - ", pf$CI.2, ")"))



tiff("plot_fixeff_significant.only.tiff", width = 3.307*3, height = 3.307*2, units = 'in', res = 600); plot_fixeff
dev.off()


## Variation in random effect for fixed effect coefficients
post_coefficient_reffvar <- data.frame(par_str= rep(par_str, each=12), genus= rep(par_name, each=12), r_str = rep(paste0("r.", 1:12), 4) , covar = rep(covar_name[-1], 4), variance_mean=rep(NA, 4*12))
...
post_coefficient_reffvar.sel <-  post_coefficient_reffvar %>% filter( variance_mean>0)
nvarcoeff <- dim(post_coefficient_reffvar.sel)[1]
post_varcoeff <- data.frame(
	effect.no = rep(1:nvarcoeff, each=nrealm),
	par_str = rep(post_coefficient_reffvar.sel$par_str, each=nrealm),
	genus = rep(post_coefficient_reffvar.sel$genus, each=nrealm),
	covar = rep(post_coefficient_reffvar.sel$covar, each=nrealm),
	r_str = rep(post_coefficient_reffvar.sel$r_str, each=nrealm),
	realm = rep(realm_name, nvarcoeff))

post_varcoeff$effect <- paste(post_varcoeff$genus, "~", post_varcoeff$covar)

for(i in 1:nvarcoeff){
	selrow <- which(post_varcoeff$effect.no==i)
	sel.1 <- selrow[1]
	mod <- get(paste0(tail(models_str,1), post_varcoeff$par_str[sel.1]))
	fixeff.mean <- post_fixeff %>% filter(genus==post_varcoeff$genus[selrow[1]] & covar==post_varcoeff$covar[sel.1])
	fixeff.mean <- fixeff.mean$mean
	reff <- which(names(mod$summary.random)==post_varcoeff$r_str[sel.1])
	post_varcoeff$coef.adj_mean [selrow] <- fixeff.mean + mod$summary.random[[reff]][, "mean"]
	post_varcoeff$coef.adj_CI.1 [selrow] <- fixeff.mean + mod$summary.random[[reff]][, "0.025quant"]
	post_varcoeff$coef.adj_CI.2 [selrow] <- fixeff.mean + mod$summary.random[[reff]][, "0.975quant"]
}
...
tiff("plot_varcoeff.tiff", width = 3.307*3, height = 3.307*2, units = 'in', res = 600); plot_varcoeff
dev.off()
...
tiff("plot_varcoeff.oppose.tiff", width = 3.307*3, height = 3.307*2, units = 'in', res = 600); plot_varcoeff.oppose
dev.off()
